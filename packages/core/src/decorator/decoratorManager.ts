...
import { GroupModeType, IModuleStore, InjectModeEnum, ObjectDefinitionOptions, ObjectIdentifier, ParamDecoratorOptions, TSDesignType, TagClsMetadata, TagPropsMetadata } from '../interface';
import { INJECT_CUSTOM_PARAM, INJECT_CUSTOM_PROPERTY, INJECT_TAG, OBJ_DEF_CLS, TAGGED_CLS } from './constants';
import { generateRandomId, merge } from '../util/index';
...
if (typeof global === 'object') {
  if (global['MIDWAY_GLOBAL_DECORATOR_MANAGER']) {
    console.warn(
      'DecoratorManager not singleton and please check @island/core version by "npm ls @island/core"'
    );
    const coreModulePathList = getModuleRequirePathList('@island/core');
  ...
  if (isProvide(target)) {
    if (identifier) {
      ...
      if (meta.id !== identifier) {
        ...
        console.debug(`update provide: ${target.name} -> ${meta.uuid}`);
      }
    }
  } else {
    // save
    ...
    console.debug(`save provide: ${target.name} -> ${uuid}`);
  }
