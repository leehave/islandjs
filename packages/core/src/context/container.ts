...
export class MidwayContainer implements IMidwayContainer, IModuleStore {
  ...
  bind<T>(identifier: any, target: any, options?: any): void {
    ...
    if (objDefOptions.initMethod) {
      debugBind(
        `${' '.repeat(debugSpaceLength)}register initMethod = ${objDefOptions.initMethod
        }`
      );
    ...
    if (objDefOptions.destroyMethod) {
      debugBind(
        `${' '.repeat(debugSpaceLength)}register destroyMethod = ${objDefOptions.destroyMethod
        }`
      );
    ...
    if (objDefOptions.allowDowngrade) {
      debugBind(
        `${' '.repeat(debugSpaceLength)}register allowDowngrade = ${objDefOptions.allowDowngrade
        }`
      );
